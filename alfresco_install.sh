...
# Determina o diretório de instalação a partir do primeiro argumento. "${foo} World"

installation_dir="$1"

# Verifica a existência da variável de ambiente JAVA_HOME. Se não estiver definida, exibe uma mensagem de erro e sai.
(( ${#JAVA_HOME} == 0 )) && echo "Erro: Varável de ambiente JAVA_HOME não foi configurada." && exit

# Verifica se o diretório especificado como argumento não está vazio. Se estiver vazio, exibe uma mensagem de erro e sai.
if [ -z "$1" ]; then
  echo "Especifique o diretório de instalação."
  exit
elif [ ! -d "${installation_dir}" ]; then
  echo "Diretório ${installation_dir} não existe"
  exit
fi

cd "$installation_dir"

# Verifica se o diretório de instalação não está vazio. Se não estiver vazio, pergunta ao usuário se deseja continuar.
stuff=$(ls "$installation_dir")
if (( ${#stuff} != 0 )); then
  echo "Diretório de instalação especificado não está vazio. Pressione Ctrl-C ou ENTER para continuar."
  read
fi
...
if [[ $ver > "6.2" && $jdk < "17" ]]; then
  echo ACS 6.2 or higher requires JAVA_HOME set to JDK 17 or higher
  exit
fi
...
wget --no-check-certificate https://jdbc.postgresql.org/download/postgresql-42.5.4.jar
cd ../..
